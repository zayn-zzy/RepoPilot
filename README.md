...
| Hybrid Retrieval（词法 BM25/语义神经 embedding/结构图/融合重排/上下文构建） | ✅ 语义栈默认真实神经模型（本地 fastembed bge-small，可配 API），LSA 为诚实标注的确定性回退；向量缓存落盘 | `mini_claude/retrieval/` |
...
```bash
python -m pytest python/tests/ -q      # 当前 504/504
```
